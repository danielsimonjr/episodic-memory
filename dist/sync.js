...
function summaryFailPath(filePath) {
    return filePath.replace('.jsonl', '-summary.failed');
}
function readSummaryAttempts(filePath) {
    try {
 ...
 */
export function isTerminalFailRecord(filePath) {
    try {
        const data = JSON.parse(fs.readFileSync(summaryFailPath(filePath), 'utf-8'));
        if (data.reason || data.gaveUp)
            return true;
        if (typeof data.lastError === 'string' && data.lastError.startsWith('skipped:'))
            return true;
        return false;
    }
    catch {
...
export async function syncConversations(sourceDir, destDir, options = {}) {
        ...
        try {
            ...
            for (const file of filesToIndex) {
                try {
                    ...
                    const embeddings = await generateExchangeEmbeddings(newExchanges.map((exchange) => ({
    ...
    if (!options.skipSummaries && filesToSummarize.length > 0) {
        ...
        const tier = (f) => (f.legacyEmpty ? 2 : f.priorFailures > 0 ? 1 : 0);
        ...
        result.pendingSummaries = filesToSummarize.filter(f => !fs.existsSync(f.path.replace('.jsonl', '-summary.txt'))).length;
    }
    return result;
}
/**
 ...
 */
export async function summarizeOneFile(filePath, sessionId, result, summarize) {
    const { parseConversation } = await import('./parser.js');
    try {
        ...
        catch { } // clear any prior failure record
        result.summarized++;
    }
    catch (error) {
