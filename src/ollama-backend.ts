...
export function summarizerBackend(): 'claude' | 'ollama' {
...
export async function callOllama(prompt: string, opts: OllamaCallOptions = {}): Promise<string> {
  ...
  try {
    ...
    if (!res.ok) {
      let detail = '';
      try { detail = JSON.stringify(await res.json()); } catch { /* body is not JSON */ }
