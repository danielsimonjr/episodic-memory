...
- Background sync skips the resume attempt entirely for conversations whose session transcript is no longer present in `~/.claude/projects/`. The resume-then-fallback fix already recovered such cases, but it still spawned one doomed resume subprocess per archived conversation before falling back. `summarizeConversation` now checks `isSessionResumable()` first and goes straight to transcript-text summarization when the session is gone, halving the SDK calls during a backlog drain. The try/catch fallback remains for the rare case where the transcript exists but resume fails anyway.
