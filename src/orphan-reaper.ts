 ...
 */
export function selectOrphans(procs: ProcSnapshot[], baselinePids: Set<number>): ProcSnapshot[] {
  ...
  const forks = fresh.filter(
    (p) => !DAEMON.test(p.commandLine) && !PTYHOST.test(p.commandLine) && FORK.test(p.commandLine)
  );
