 ...
 */
export function resolveArchiveJsonlPath(candidatePath) {
    ...
    const lexicalInside = lexicalNorm === archiveNorm ||
    ...
    if (!lexical.toLowerCase().endsWith('.jsonl')) {
        throw new Error(`Path must point to a .jsonl conversation file: ${candidatePath}`);
    }
    ...
    try {
        ...
        throw new Error(`Unable to resolve path: ${candidatePath} (${err instanceof Error ? err.message : String(err)})`);
    }
    const realNorm = normalizeForCompare(realPath);
    const realInside = realNorm === archiveNorm || realNorm.startsWith(archiveNorm + path.sep);
    if (!realInside) {
        throw new Error(`Path resolves outside the conversation archive (symlink escape rejected): ${candidatePath}`);
    }
 ...
 */
export async function readJsonlLines(filePath, startLine, endLine) {
    const stat = fs.statSync(filePath);
    const hasRange = startLine !== undefined || endLine !== undefined;
    const cap = maxReadBytes();
    if (!hasRange && stat.size > cap) {
        throw new Error(`File is ${stat.size} bytes, which exceeds the ${cap}-byte MCP read cap. ` +
    ...
    try {
        for await (const line of rl) {
            ...
            if (bytesAccumulated > cap) {
                throw new Error(`Selected line range exceeds the ${cap}-byte MCP read cap. ` +
